...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            @if($productsAlerte->count() > 0)
                <div class="space-y-3">
                    @foreach($productsAlerte as $product)
                    <div class="flex items-center justify-between p-3 bg-red-50 rounded border-l-4 border-red-500">
                        <div>
                            <p class="font-semibold">{{ $product->name }}</p>
                            <p class="text-sm text-gray-600">{{ $product->format }} - {{ $product->categorie->name }}</p>
                        </div>
                        <div class="text-right">
                            <p class="text-red-600 font-bold">{{ $product->stock_actuel }} unités</p>
                            <p class="text-xs text-gray-500">Seuil: {{ $product->seuil_alerte }}</p>
                        </div>
                    </div>
                    @endforeach
                </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between p-3 bg-gray-50 rounded">
                        <div class="flex items-center gap-3">
                            ...
                            <p class="font-semibold">{{ $vente->name }}</p>
                        </div>
